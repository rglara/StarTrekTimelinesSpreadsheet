...
export interface VoyageWorkerMessage {
	//TODO: receive only necessary data instead of the whole DTO
	vd: VoyageDescriptionDTO;
	roster: CrewData[];
	shipAM: number;
}

export interface VoyageWorkerResult {
	choices: CalcChoice[];
	hoursLeft: number;
}
...
function calculateVoyage(options: {
	vd: VoyageDescriptionDTO;
	roster: CrewData[];
	shipAM: number;
},
