...
class NumberPicker extends React.Component {
	...
	handleAction(event) {
		...
		var setVal = isFinite(parseFloat(this.props.value)) ? parseFloat(this.props.value) : null;
		let stepSize = isFinite(parseFloat(this.props.step)) ? parseFloat(this.props.step) : 1;
		switch (actionFilter) {
			case DECREASE_VALUE:
		...
		let precision = 1000;
		if (isFinite(parseFloat(setVal))) returnValue = Math.floor(parseFloat(setVal) * precision) / precision;
	...
	validateInput(event, v) {
		...
		switch (actionFilter) {
			case this.props.name:
				let parsedVal = parseFloat(currentValue);
				setVal = isFinite(parsedVal) ? parsedVal : null;
