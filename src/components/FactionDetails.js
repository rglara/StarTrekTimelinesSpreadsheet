...
class FactionDisplay extends React.Component {
	...
	_buyItem(storeItem) {
		...
		STTApi.executePostRequestWithUpdates('commerce/buy_direct_offer', { id, layout: this.props.faction.shop_layout, e: 0 }).then(
			buyData => {
				this.refreshStore();
			}
		);
		...
		return (
			<div
				...
				{this.state.storeItems.map((storeItem, idx) => (
					<StoreItem key={idx} storeItem={storeItem} onBuy={() => this._buyItem(storeItem)} />
				))}
			</div>
		);
	}
